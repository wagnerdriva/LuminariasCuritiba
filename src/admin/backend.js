...
export async function executeAPI(command, option, fields, parse = true) {
  ...
  return new Promise((resolve, reject) => {
    if (request.status === 200)
      if (parse) resolve(JSON.parse(request.response));
      else resolve(request.response);
    else reject({ status: request.status, statusText: request.statusText });
  });
}
